...
<table class="table table-striped key-threats">
    ...
    <g:each in="${(0..<max)}" var="i">
        <tr>
            ...
            <td><fc:renderComparison changed="${changed.custom.details.threats.rows ?: []}" i="${i}" original="${project.custom.details.threats.rows ?: []}" property="threat"/> </td>
            <td><fc:renderComparison changed="${changed.custom.details.threats.rows ?: []}" i="${i}" original="${project.custom.details.threats.rows ?: []}" property="intervention"/> </td>
        </tr>
    </g:each>
