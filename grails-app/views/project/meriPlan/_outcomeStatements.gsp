...
<table class="table ${extendedOutcomes ? 'extended' :''}">
    ...
    <tr>
        ...
        <g:if test="${extendedOutcomes}">
            ...
            <g:else>
            <td class="investment-priority">
                <select class="form-select form-select-sm" data-validation-engine="validate[required]" data-bind="options:details.outcomes.selectedPrimaryAndSecondaryPriorities,  optionsText:'name', optionsValue:'investmentPriorityId', value:asset, optionsCaption:'Please select...', disable: $parent.isProjectDetailsLocked()"></select>
            </td>
            </g:else>
