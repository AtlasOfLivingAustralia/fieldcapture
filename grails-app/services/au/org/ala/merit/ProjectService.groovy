...
class ProjectService  {
    ...
    private String validateProjectEndDate(Map project, ProgramConfig config, String plannedEndDate, ReportGenerationOptions options) {
        ...
        if (config.activityBasedReporting) {
            ...
            if (lastReport && plannedEndDateAlignedToReportingSchedule < lastReport.fromDate) {
                message = "The project end date must be on or after ${DateUtils.isoToDisplayFormat(lastReport.fromDate)}"
            }
        }
        message
    }
    ...
    String minimumProjectEndDate(Map project, Map projectConfiguration) {
        ...
        if (projectConfiguration.autogeneratedActivities) {
            project.reports.each {
                if (it.progress != ActivityService.PROGRESS_PLANNED) {
                    println "report.progress: " + it.progress
                    println "report.fromDate: " + it.fromDate
                }
            }
            minimumDate = project.reports?.findAll{it.progress != ActivityService.PROGRESS_PLANNED}?.max{it.fromDate}?.fromDate
//            minimumDate = project.reports?.findAll{excludesNotApproved(it)}?.max{it.toDate}?.toDate
            // We allow reports to be generated up to 24 hours after the end of a project due to
            // project end dates being 00:00 of the last day of the project instead of 23:59:..
            if (minimumDate) {
...
//        }
    ...
    */
    ...
    boolean excludesNotApproved(Map report) {
        return report.publicationStatus == ReportService.REPORT_SUBMITTED || report.publicationStatus == ReportService.REPORT_APPROVED || report.publicationStatus == ReportService.REPORT_CANCELLED
    }
}
