...
class ReportServiceSpec extends Specification implements AutowiredTest{
    ...
    void "no stage overlap should occur when an existing approved report exists with misaligned times"() {
        ...
        1 * webService.doPost({it.endsWith('/report/2')}, [fromDate: '2016-07-01T00:00:00Z', toDate: '2016-12-31T13:00:00Z', submissionDate: '2016-12-31T13:00:00Z', type: 'Activity', name: 'Stage 2', description: 'Stage 2 for project', projectId: 'p1', reportId:'2', category:'Stage reports', 'generatedBy':'Stage reports', 'generatedBy':'Stage reports'])

        and: "a new report is added to cover the project end date extension"
        1 * webService.doPost({it.endsWith('/report/')}, [fromDate: '2016-12-31T13:00:00Z', toDate: '2017-06-29T14:00:00Z', submissionDate: '2017-06-29T14:00:00Z', type: 'Activity', name: 'Stage 3', description: "Stage 3 for project", projectId: 'p1', category:'Stage reports', 'generatedBy':'Stage reports', 'generatedBy':'Stage reports'])
