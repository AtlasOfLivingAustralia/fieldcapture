...
class ProgramControllerSpec extends Specification implements ControllerUnitTest<ProgramController>{
    ...
    def "program reports can be regenerated by delegating the service with categories"() {
        when:
        request.method = "POST"
    ...
    def "program reports can be regenerated by delegating the service without categories"() {
         when:
        request.method = "POST"
        request.JSON = [:]
        controller.regenerateProgramReports("p1")
